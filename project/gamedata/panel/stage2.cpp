...
void Stage2::LoadMap() {
	FILE* fp = nullptr;
	char fname[] = "project/gamedata/resources/csv/Map2Panel.csv";
	errno_t err;
...
void Stage2::LoadDirection() {
	int test = 0;

	FILE* fp = nullptr;
	char fname[] = "project/gamedata/resources/csv/Map2Direction.csv";
	errno_t err;
	err = fopen_s(&fp, fname, "r");
	for (int i = 0; i < kMapHeight; i++) {
		for (int j = 0; j < kMapWidth; j++) {
...
void Stage2::LoadEvent() {
	FILE* fp = nullptr;
	char fname[] = "project/gamedata/resources/csv/Map2Event.csv";
	errno_t err;
	err = fopen_s(&fp, fname, "r");
	for (int i = 0; i < kMapHeight; i++) {
		for (int j = 0; j < kMapWidth; j++) {
...
void Stage2::FoldDirecting(int direction, bool isLapel) {
	drawTest_ = true;
...
int Stage2::GetNowMapState(Vector2 pos) {
	int i = 0;
	int j = 0;
	int state = 0;
...
int Stage2::GetNowMapStatePosX() {
	return playerStatePosX_;
}

int Stage2::GetNowMapStatePosY() {
	return playerStatePosY_;
}

int Stage2::GetStartPosX() {
	return startPointX_;
}

int Stage2::GetStartPosY() {
	return startPointY_;
}
