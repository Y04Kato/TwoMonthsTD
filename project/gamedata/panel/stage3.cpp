...
void Stage3::LoadMap() {
	FILE* fp = nullptr;
	char fname[] = "project/gamedata/resources/csv/Map3Panel.csv";
	errno_t err;
...
void Stage3::LoadDirection() {
	int test = 0;

	FILE* fp = nullptr;
	char fname[] = "project/gamedata/resources/csv/Map3Direction.csv";
	errno_t err;
	err = fopen_s(&fp, fname, "r");
	for (int i = 0; i < kMapHeight; i++) {
		for (int j = 0; j < kMapWidth; j++) {
...
void Stage3::LoadEvent() {
	FILE* fp = nullptr;
	char fname[] = "project/gamedata/resources/csv/Map3Event.csv";
	errno_t err;
	err = fopen_s(&fp, fname, "r");
	for (int i = 0; i < kMapHeight; i++) {
		for (int j = 0; j < kMapWidth; j++) {
...
void Stage3::FoldDirecting(int direction, bool isLapel) {
	drawTest_ = true;
...
int Stage3::GetNowMapState(Vector2 pos) {
	int i = 0;
	int j = 0;
	int state = 0;
...
int Stage3::GetNowMapStatePosX() {
	return playerStatePosX_;
}

int Stage3::GetNowMapStatePosY() {
	return playerStatePosY_;
}

int Stage3::GetStartPosX() {
	return startPointX_;
}

int Stage3::GetStartPosY() {
	return startPointY_;
}
