...
void Stage5::LoadMap() {
	FILE* fp = nullptr;
	char fname[] = "project/gamedata/resources/csv/Map5Panel.csv";
	errno_t err;
...
void Stage5::LoadDirection() {
	int test = 0;

	FILE* fp = nullptr;
	char fname[] = "project/gamedata/resources/csv/Map5Direction.csv";
	errno_t err;
	err = fopen_s(&fp, fname, "r");
	for (int i = 0; i < kMapHeight; i++) {
		for (int j = 0; j < kMapWidth; j++) {
...
void Stage5::LoadEvent() {
	FILE* fp = nullptr;
	char fname[] = "project/gamedata/resources/csv/Map5Event.csv";
	errno_t err;
	err = fopen_s(&fp, fname, "r");
	for (int i = 0; i < kMapHeight; i++) {
		for (int j = 0; j < kMapWidth; j++) {
...
void Stage5::FoldDirecting(int direction, bool isLapel) {
	drawTest_ = true;
...
int Stage5::GetNowMapState(Vector2 pos) {
	int i = 0;
	int j = 0;
	int state = 0;
...
int Stage5::GetNowMapStatePosX() {
	return playerStatePosX_;
}

int Stage5::GetNowMapStatePosY() {
	return playerStatePosY_;
}

int Stage5::GetStartPosX() {
	return startPointX_;
}

int Stage5::GetStartPosY() {
	return startPointY_;
}
